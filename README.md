...
- Для работы агента в сети **Ethereum mainnet** задайте `NETWORK_ID=ethereum-mainnet` и переменную `ETHEREUM_RPC_URL` с HTTPS RPC-URL (Infura, Alchemy, QuickNode и т.п.). В этом режиме баланс кошелька и реальные сделки будут выполняться в сети Ethereum mainnet с использованием импортированного приватного ключа.
...
- При создании агента в режиме WALLET в форме создания (`/dashboard/agents/create`) можно сразу задать приватный ключ кошелька в поле **«Приватный ключ 0x...»** — тогда агент сразу будет работать с этим реальным кошельком.
